...
int main(void) {
		...
		//Rendering Scope
		{
			...
			Camera cam(window);
			UiElement crosshair(glm::vec2(896/2, 504/2), glm::vec2(16,16), "res/textures/Crossair.png");

			int s = (896 - (9 * 40))/2 + 20;
			std::vector<UiElement*> slot;
			std::vector<UiElement*> slotSelected;

			for (size_t i = 0; i < 9; i++) {
				slot.push_back( new UiElement(glm::vec2(s + 37 * i, 20), glm::vec2(40, 40), "res/textures/Slot.png"));
				slotSelected.push_back( new  UiElement(glm::vec2(s + 37 * i, 20), glm::vec2(40, 40), "res/textures/SlotSelected.png"));
			}
			...
			while (!glfwWindowShouldClose(window) && !endApp) {
				...
				if (!firstFrame) deltaTime = currentFrame - lastFrame;
